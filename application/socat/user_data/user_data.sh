...
docker run -d --privileged --security-opt seccomp=unconfined --device /dev/net/tun --device /dev/vsock --restart unless-stopped --name http_server -v /etc/pki/tls/certs/:/etc/pki/tls/certs/ -p 443:443 ${__SIGNING_SERVER_IMAGE_URI__}
